...
public class SDLActivity extends Activity implements View.OnSystemUiVisibilityChangeListener {
    ...
    private static final boolean S2HD_GAME_PORT = true;
...
*/
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!mBrokenLibraries) {
            ...
            if (!version.equals(expected_version)) {
                Log.w(TAG, "SDL C/Java version mismatch (expected " + expected_version + ", got " + version + "); continuing (S2HD port).");
            }
        }
    ...
    protected static class SDLCommandHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            case COMMAND_CHANGE_TITLE:
                if (!S2HD_GAME_PORT && context instanceof Activity) {
                    ((Activity) context).setTitle((String)msg.obj);
                } else if (!S2HD_GAME_PORT) {
                ...
                if (Build.VERSION.SDK_INT >= 19 /* Android 4.4 (KITKAT) */) {
                    if (context instanceof Activity) {
                        ...
                        if (window != null) {
                            ...
                            if (S2HD_GAME_PORT && context instanceof SDLActivity) {
                                ((SDLActivity) context).applyImmersiveFullscreenNow();
                            }
                        }
                    } else {
                        Log.e(TAG, "error handling message, getContext() returned no Activity");
    ...
    /**
     * Immersive fullscreen on the Activity window (runs on UI thread). SDL posts similar work
     * asynchronously; this keeps bars hidden when native toggles window style.
     */
    protected void applyImmersiveFullscreenNow() {
        if (Build.VERSION.SDK_INT < 19 /* Android 4.4 (KITKAT) */) {
            return;
        }
...
*/
    ...
    public static boolean showTextInput(int x, int y, int w, int h) {
        if (S2HD_GAME_PORT) {
            /* Tell SDL the request succeeded but do not create DummyEdit / IME (action game). */
            return true;
        }
